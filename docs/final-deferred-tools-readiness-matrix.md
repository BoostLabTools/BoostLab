...
| Driver Install Debloat & Settings | `driver-install-debloat-settings` | Graphics | `source-ultimate/5 Graphics/1 Driver Install Debloat & Settings.ps1` | `E69EFF538E7CE6108233C525A2BB88BA2D549CE6954AE751BE7BED778271C26F` | Implemented manual handoff; Auto blocked; Phase 122 branch-scope decision recorded | `docs/tool-designs/driver-install-debloat-settings-scope-provenance-design.md`; `docs/migrations/driver-install-debloat-settings.md` | Missing exact three-branch source-equivalent implementation and driver/profile rollback approval for Auto | Manual handoff implemented; download provenance foundation; installer policy; driver rollback foundation; reboot recovery foundation; AppX foundation; cleanup policy; file/registry rollback foundation; Phase 122 approves NVIDIA/AMD/INTEL branches for this tool only | Exact NVIDIA/AMD/Intel artifacts, driver/package scopes, service/task/process scopes, profile import rollback, cleanup scopes, AppX/package scopes, registry scopes, reboot workflow | Tool-specific NVIDIA/AMD/Intel artifact approvals; driver scopes; service/task/process scopes; profile scopes; cleanup scopes; AppX/package scopes; registry scopes; reboot scope | Unavailable | Restore unavailable until driver/profile/package/registry/file/service/task/reboot state is captured and approved | Shared Windows behavior only; Windows 10 optimization branches unsupported | Tool-specific exception: NVIDIA/AMD/INTEL branches approved for this tool only; project-wide AMD/Intel scope unchanged | High | Pending exact parity implementation | Implement exact source-equivalent NVIDIA, AMD, and INTEL branch behavior with mocks/descriptors; keep Auto blocked until complete | P13 |
...
| Visual C++ | `visual-cpp` | Graphics | `source-ultimate/5 Graphics/3 C++.ps1` | `7ACB1F25ECFEEAD83FA389E2D0C1FEEF12232C4E9A740CB5DE64A326FFD38C09` | Implemented manual handoff; Auto blocked | `docs/visual-cpp-provenance-review.md`; `docs/migrations/visual-cpp.md` | Missing artifact provenance for Auto | Manual handoff implemented; download provenance foundation; installer policy | Auto still needs immutable sources, exact hash/size/version/signer evidence for all packages, exit-code rules, temp ownership scopes | Future Auto artifact approvals; installer descriptors; generated-temp-path scopes | Unavailable | Restore unavailable until captured-state restore contract exists | No Windows 10 branch issue documented | Not GPU-specific | High | Complete for manual handoff only | Auto remains blocked until all twelve redistributable approvals exist | P11 |
...
* `start-menu-taskbar`, `cleanup`, and most Windows tools are blocked by governance, not by host OS alone, unless a future source branch is explicitly Windows 10-only.
