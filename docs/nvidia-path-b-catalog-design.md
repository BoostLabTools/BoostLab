...
Phase 93 current-state note: `Driver Install Latest` has since been promoted
as an active controlled manual-handoff tool for Path B step 1 only. This does
not approve NVIDIA driver downloads, installer execution, artifact provenance,
driver mutation, browser opening, external process launch, reboot/session
