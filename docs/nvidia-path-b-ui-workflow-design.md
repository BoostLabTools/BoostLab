...
It keeps Path B catalog/schema-only and adds no WPF/UI implementation or
...
It defines future read-only preview metadata and sample fields without adding
live catalog UI, active UI config, runtime behavior, or action enablement.
...
It defines future fail-closed drift handling for preview data without modifying
WPF/UI runtime files or enabling any Path B control.
...
adds no live UI navigation, WPF changes, action enablement, or runtime behavior.
