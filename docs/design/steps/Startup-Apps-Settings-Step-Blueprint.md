# AXIS Startup Apps Settings Step Blueprint
...
This document records Yazan-approved Arabic-first customer-facing content for the `startup-apps-settings` Setup-stage step.
...
| Field | Value |
| --- | --- |
| Internal tool ID | `startup-apps-settings` |
| Customer-facing step title | `Startup Apps Settings` |
| Stage | `Setup` |
| Visible stage label | `Setup` |
| Production action mapping | customer-facing `Ø§ÙØªØ­` maps later to internal `Open` for `startup-apps-settings` |
