# AXIS Visual C++ Runtimes Step Blueprint
...
This document records Yazan-approved Arabic-first customer-facing content for the `visual-cpp` Graphics-stage step.

This phase is documentation-only. It does not implement UI behavior, download anything, run Visual C++ redistributable installers, install runtime components, modify files, registry, system settings, Windows components, wire the step into `ui/MainWindow.ps1`, edit the isolated prototype, edit tests, edit config, or change runtime contracts.
...
| Field | Value |
| --- | --- |
| Internal tool ID | `visual-cpp` |
| Customer-facing step title | `Visual C++ Runtimes` |
| Stage | `Graphics` |
| Visible stage label | `Graphics` |
| Production action mapping | customer-facing `ØªØ«Ø¨ÙŠØª Ù…ÙƒÙˆÙ†Ø§Øª Visual C++` maps later to internal `Apply` for `visual-cpp` |
| Prototype behavior | Simulation only. Do not download anything. Do not run Visual C++ redistributable installers. Do not install runtime components. Do not modify files, registry, system settings, or Windows components. |
...
| Subtitle | `ØªØ«Ø¨ÙŠØª Ù…ÙƒÙˆÙ†Ø§Øª ØªØ´ØºÙŠÙ„ Ø¶Ø±ÙˆØ±ÙŠØ© Ù„Ù„Ø£Ù„Ø¹Ø§Ø¨ ÙˆØ§Ù„Ø¨Ø±Ø§Ù…Ø¬.` |
| Primary action button | `ØªØ«Ø¨ÙŠØª Ù…ÙƒÙˆÙ†Ø§Øª Visual C++` |
| Information card title | `Ù…ÙƒÙˆÙ†Ø§Øª ØªØ´ØºÙŠÙ„ Ø§Ù„Ø£Ù„Ø¹Ø§Ø¨ ÙˆØ§Ù„Ø¨Ø±Ø§Ù…Ø¬` |
| Information card bullet 1 | `ØªØ«Ø¨ÙŠØª Ù…ÙƒÙˆÙ†Ø§Øª Visual C++ Ø§Ù„Ø¶Ø±ÙˆØ±ÙŠØ© Ù„ØªØ´ØºÙŠÙ„ Ø§Ù„Ø¹Ø¯ÙŠØ¯ Ù…Ù† Ø§Ù„Ø£Ù„Ø¹Ø§Ø¨ ÙˆØ§Ù„Ø¨Ø±Ø§Ù…Ø¬.` |
| Information card bullet 2 | `ØªØ³Ø§Ø¹Ø¯ Ù‡Ø°Ù‡ Ø§Ù„Ù…ÙƒÙˆÙ†Ø§Øª Ø¹Ù„Ù‰ ØªØ­Ø³ÙŠÙ† ØªÙˆØ§ÙÙ‚ Ø§Ù„Ø¨Ø±Ø§Ù…Ø¬ Ø§Ù„ØªÙŠ ØªØ­ØªØ§Ø¬ Ù…ÙƒØªØ¨Ø§Øª ØªØ´ØºÙŠÙ„ Ø¥Ø¶Ø§ÙÙŠØ©.` |
...
Do not show installer names.

Do not show versions.
...
The prototype must not download anything, run Visual C++ redistributable installers, install runtime components, modify files, registry, system settings, Windows components, Defender, USB, boot settings, firmware settings, host configuration, or run Apply, Open, Default, Restore, or Analyze.
...
`Visual C++` inside Arabic lines must be rendered safely.

## Non-Goals

This phase does not include UI implementation, runtime implementation, prototype changes, tests, config, modules, source changes, real Apply/Open/Default/Restore/Analyze actions, downloads, installer execution, runtime installation, Registry mutation, host mutation, staging, committing, or pushing.
