...
Phase 124 current-state note: `Driver Install Latest` is now implemented as a
source-equivalent, branch-selected runtime for its source-defined NVIDIA, AMD,
and INTEL branches, with BoostLab GUI confirmation and test-safe executor
injection. `Nvidia Settings` remains controlled manual handoff. No reusable
global artifact provenance, production allowlist, or cross-tool AMD/Intel scope
is approved by the Driver Install Latest implementation.
...
|---:|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Driver Install Latest | `source-ultimate/_intake-promoted/Ultimate/5 Graphics/2 Driver Install Latest.ps1` | `41C9DEA9AA5D208C9ED1EB7F1512B24251FBF4DC01C6DE2858B5B1A26C631A2F` | `5 Graphics/2 Driver Install Latest.ps1` | Graphics | Prompts for GPU vendor; NVIDIA branch queries NVIDIA driver API, builds latest driver URL, downloads to `%SystemRoot%\Temp\nvidiadriver.exe`, then launches installer; AMD branch scrapes AMD support, downloads `%SystemRoot%\Temp\amddriver.exe`, and launches it; INTEL branch opens the Intel Windows 11 graphics driver search page. | `Invoke-WebRequest`/`IWR`, `ConvertFrom-Json`, `Start-Process`, `Read-Host`, console output, admin self-elevation. | None. | `%SystemRoot%\Temp\nvidiadriver.exe`; `%SystemRoot%\Temp\amddriver.exe`. | NVIDIA driver API and NVIDIA driver installer URL; AMD web installer branch; Intel driver webpage branch. | NVIDIA driver installer executable; AMD installer branch; Intel browser handoff branch. | Process launch/handoff to driver installer or driver page. No service/task mutation directly in script. | Source-defined latest driver handoff for NVIDIA, AMD, and INTEL in this tool only. | No explicit reboot command, but vendor driver installers can have reboot/session implications. | No source Default or Restore path for the driver installer. | Download, installer, driver mutation, process handoff, admin, internet, reboot-capable installer. | Action Plan confirmation; test-safe executor injection; source-equivalent runtime mapping. No reusable global production artifact scope is approved here. | Phase 124 source-equivalent runtime. | NearParityControlled / DoneYazanAcceptedNearParity |
...
The source selects, downloads, launches, or hands off to GPU driver installers.
Phase 124 maps all three source-defined branches for this tool only:
NVIDIA latest-driver lookup/download/installer launch, AMD support-page scrape/
download/installer launch, and INTEL driver search page launch. This does not
expand project-wide AMD/Intel GPU scope.
