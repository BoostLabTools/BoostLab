...
injection. This does not create reusable/global artifact provenance,
production allowlists, or cross-tool AMD/Intel GPU scope. `Nvidia Settings`
remains controlled manual handoff.
...
| 1 | Driver Install Latest | AMD driver web installer branch | Source AMD branch `Invoke-WebRequest` and `IWR $DownloadAmd.href` | `https://www.amd.com/en/support/download/drivers.html` and `drivers.amd.com/...minimalsetup..._web.exe` | HTTPS page scrape/download | Installer | `.exe` | Official AMD-looking dynamic source; Phase 124 maps it for Driver Install Latest only | No | No | No | No | Yes, `%SystemRoot%\Temp\amddriver.exe` | No | Yes | No | ToolScopedRuntimeMapped | Phase 124 preserves the source-defined AMD branch only inside Driver Install Latest; it does not approve reusable AMD artifact provenance or project-wide AMD scope | Keep branch mapping tool-scoped; no reusable/global artifact approval exists |
| 1 | Driver Install Latest | Intel driver page branch | Source Intel branch `Start-Process "https://www.intel.com/..."` | Intel driver search page URL | Browser handoff | Web page | URL | Official Intel-looking page; Phase 124 maps it for Driver Install Latest only | No | No | No | No | Not applicable | No | Browser handoff only | No | ToolScopedRuntimeMapped | Phase 124 preserves the source-defined INTEL branch only inside Driver Install Latest; it does not approve reusable Intel artifact provenance or project-wide Intel scope | Keep branch mapping tool-scoped; no reusable/global artifact approval exists |
