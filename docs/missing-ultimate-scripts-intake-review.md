...
then superseded in Phase 124 by source-equivalent branch-selected runtime for
this tool only. Nvidia Settings was promoted in Phase 94 as controlled manual
handoff only. HDCP was promoted in Phase 95 as controlled NVIDIA-only registry behavior
...
* `Driver Install Latest.ps1` contains NVIDIA, AMD, and INTEL latest-driver branches. Phase 124 implements the source-equivalent branch-selected runtime for this tool only; reusable/global artifact provenance, standalone driver artifact approval, Default, Restore, and cross-tool AMD/Intel scope remain unapproved.
...
* AMD and Intel GPU-specific branches remain unsupported except for explicit tool-specific Yazan decisions such as the Phase 124 Driver Install Latest source-equivalent branch mapping.
...
The intake graphics scripts are mostly NVIDIA-specific or driver-focused. Future promotion must preserve NVIDIA-only scope unless Yazan explicitly records a tool-specific exception. Phase 124 is such an exception for `Driver Install Latest.ps1` only and does not broaden project-wide AMD/Intel scope.
...
| `5 Graphics/2 Driver Install Latest.ps1` | Implemented as source-equivalent branch-selected runtime in Phase 124 | Reusable/global artifact provenance, Default, Restore, and cross-tool AMD/Intel scope remain unapproved | downloads/artifacts/installers; driver install/profile/settings; NVIDIA/AMD/INTEL tool-specific branch behavior; process/browser handoff; reboot/session implications | Yazan approved all source-defined NVIDIA, AMD, and INTEL branches for Driver Install Latest only. This does not expand project-wide AMD/Intel GPU scope. | NVIDIA branch queries NVIDIA driver API, downloads the latest NVIDIA installer to `%SystemRoot%\Temp\nvidiadriver.exe`, and launches it. AMD branch downloads AMD installer; Intel branch opens Intel driver page. BoostLab preserves those source-defined operations behind explicit confirmation and test-safe executor injection. |
