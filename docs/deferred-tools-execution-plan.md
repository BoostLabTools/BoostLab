...
Phase 112 supersedes the earlier `Updates Drivers Block` live-policy final
scope. The final supported behavior is the Yazan-selected Driver Updates Block
Bootable USB branch only: Analyze, Apply to selected USB `setupcomplete.cmd`
after file state capture, unavailable Default/Unblock, and selected captured USB
file Restore. Live host registry block/unblock, broad Windows Updates, custom
update-server URL, script execution, external-process, and host reboot branches
remain blocked.
...
| `installers` | Installers | Installers | `source-ultimate/4 Installers/1 Installers.ps1` | `268C1EFE627FADDA17892223D4C35E4845833506C22AADD3240C894ED046A6F8` | Implemented selected-app queue with Yazan final app-list exception | Installer execution policy | Phase 119 implements a checkbox multi-select queue for retained source app choices only. Yazan excluded source menu entries 11 Frame View, 12 GOG launcher, 15 Notepad ++, 16 Nvidia App, 18 Onboard Memory Manager, and 19 Pot Player. Retained apps use source-derived URLs, destinations, commands, arguments, and side-effect descriptors. | Restore remains unavailable without captured package/file/registry/service/task/shortcut/config state and a selected-state Restore contract. Removed apps remain unavailable unless Yazan changes scope. | NVIDIA-only scope removed FrameView/Nvidia App from the visible catalog; most retained apps are GPU-neutral. | `Installers Retained Catalog Maintenance` | No for current final scope; Yes only for future Restore |
| `driver-install-debloat-settings` | Driver Install Debloat & Settings | Graphics | `source-ultimate/5 Graphics/1 Driver Install Debloat & Settings.ps1` | `00D7EA2C941DF776F729CD35A9386FE18D59D02717DCB3CF43282714E345A6D3` | Implemented source-equivalent three-branch controlled runtime in Phase 123 | Driver state capture and rollback | Phase 123 implements NVIDIA, AMD, and INTEL branch operation descriptors, confirmation, logging, and test-safe executor injection for this tool only. The source workflow downloads tools, debloats extracted driver contents, installs drivers, imports profiles, writes registry, deletes files, removes packages/services/tasks/processes, opens UI, and restarts. | Existing foundations plus Phase 123 tool-specific descriptors support current Apply; future Restore still needs selected captured-state restore semantics across driver/profile/package/registry/file/service/task/process/reboot state | Project-wide AMD/Intel GPU scope remains unchanged; reusable/global production artifact or allowlist approvals remain absent | `Driver Install Debloat Settings Restore Contract` | No for current Apply; Yes only for future Restore |
| `directx` | DirectX | Graphics | `source-ultimate/5 Graphics/2 DirectX.ps1` | `B944AE03DE0AFDD7329B84BBF53FF5624739465CBB7130A021E097A6723B1B27` | Implemented near parity in Phase 129 | Download provenance and checksum/signature policy | Phase 129 implements the source-equivalent DirectX workflow after explicit BoostLab confirmation and with test-safe executor injection. The source-defined `7zip.exe` and `directx.exe` URLs remain unchanged and are classified as `UltimateAuthorHostedArtifact` / `NeedsBoostLabMirror`. | Future mirror/provenance work still requires immutable artifact sources; exact hash, size, signer, license, extraction inventory, and `DXSETUP.exe` provenance before any BoostLab mirror substitution or reusable artifact approval. | Yazan accepted source-equivalent runtime with artifact source classification, not production artifact approval | `DirectX Mirror Provenance and Restore Contract`, if needed later | No; Phase 129 runtime complete |
| `visual-cpp` | Visual C++ | Graphics | `source-ultimate/5 Graphics/3 C++.ps1` | `01D6A5FAFD5E7C1FB9DA1913BD17C543EE0F8A4A7E2A7DF5583A50AEF1D82374` | Implemented near parity in Phase 130 | Download provenance and checksum/signature policy | Phase 130 implements the source-equivalent Visual C++ workflow after explicit BoostLab confirmation and with test-safe executor injection. The twelve source-defined redistributable URLs remain unchanged and classified as `UltimateAuthorHostedArtifact` / `NeedsBoostLabMirror`. | Future mirror/provenance work still requires immutable artifact sources; exact hash, size, signer, license, redistributability, installer exit-code, and temp ownership evidence before any BoostLab mirror substitution or reusable artifact approval. | Yazan accepted source-equivalent runtime with artifact source classification, not production artifact approval | `Visual C++ Mirror Provenance and Restore Contract`, if needed later | No; Phase 130 runtime complete |
| `start-menu-taskbar` | Start Menu Taskbar | Windows | `source-ultimate/6 Windows/1 Start Menu Taskbar.ps1` | `D53678CE91FE8ADE6D28F221A2E4153188597D850149F87227B26E0B821EFFF4` | Refused | File/registry state capture and rollback | Replaces `start2.bin`, deletes Quick Launch state, writes policy XML, and terminates Explorer without captured prior user state. Phase 49 scope design: `docs/tool-designs/start-menu-taskbar-scope-design.md`. | File/registry state capture and rollback; destructive cleanup policy | No scope exception | `Start and Taskbar State Migration` | Yes |
| `copilot` | Copilot | Windows | `source-ultimate/6 Windows/8 Copilot.ps1` | `45F87252A018398E87B281DE094E4943A63026567EB0782B631BBEF989CF6A9E` | Refused | AppX/package inventory and restore framework | Registry-only behavior would weaken Ultimate; full source removes/re-registers AppX and stops a broad process set. Phase 62 scope design: `docs/tool-designs/copilot-scope-design.md`. | AppX/package inventory and restore framework; process-handling policy | No scope exception | `Copilot Package and Policy Workflow` | Yes |
| `bloatware` | Bloatware | Windows | `source-ultimate/6 Windows/11 Bloatware.ps1` | `EBCE09158AB61ADE2C181DD5DB64C94B962BAF133DB4DB6122CEE642B9A48C9F` | Refused | AppX/package inventory and restore framework | Broad AppX removal/re-registration, services, downloads, installers, security-related features, and deletion with no single reversible baseline. Phase 51 scope design: `docs/tool-designs/bloatware-scope-design.md`. | AppX/package inventory and restore framework; service state capture and rollback; destructive cleanup policy; download provenance and installer execution policies | No scope exception | `Bloatware Analysis and Package Plan` | Yes |
| `game-bar` | GameBar | Windows | `source-ultimate/6 Windows/12 Gamebar.ps1` | `C35831AFE527DFA090E5DA6EBF0F6132256A4ABF3BEBDA90FC8605C47F55C0D2` | Refused | TrustedInstaller execution framework | Full source removes AppX, uninstalls GameInput, changes services/protocols, downloads repair tools, and uses TrustedInstaller. Phase 61 scope design: `docs/tool-designs/gamebar-scope-design.md`. | AppX/package inventory and restore framework; TrustedInstaller execution framework; download provenance and installer execution policies; service state capture and rollback | No scope exception | `GameBar and Gaming Services Repair` | Yes |
| `edge-webview` | Edge & WebView | Windows | `source-ultimate/6 Windows/13 Edge & WebView.ps1` | `3AB92D76307B1CB4C6988DB2201631C14D3B91B32CFFA4F1177B3E1F4F0D7966` | Implemented manual handoff; Auto blocked | Destructive cleanup policy | Phase 106 implements Analyze/Open manual handoff only. Auto remains blocked because the source removes and repairs Edge/WebView with downloads, installers, process stops, service/task deletion, file cleanup, registry mutation, Active Setup/RunOnce/BHO changes, and Windows 10 legacy package removal. Phase 52 scope design: `docs/tool-designs/edge-webview-scope-design.md`; migration record: `docs/migrations/edge-webview.md`. | Future Auto requires download provenance and checksum/signature policy; installer execution policy; package/process/service/task/file/registry cleanup scopes; destructive cleanup policy; file/registry state capture and rollback; restore/support contract | No scope exception | `Edge and WebView Removal/Repair Auto Approval Package` | No for manual handoff; Yes for Auto |
| `control-panel-settings` | Control Panel Settings | Windows | `source-ultimate/6 Windows/15 Control Panel Settings.ps1` | `F81FB649A4645A5145B43A051DDF8306145E64F1FCA5249F90B66BFDFA97BE83` | Refused | TrustedInstaller execution framework | Very large optimization source with services, deletion, security-sensitive policy, and TrustedInstaller. Current `Open` metadata understates risk. Phase 64 scope design: `docs/tool-designs/control-panel-settings-scope-design.md`. | TrustedInstaller execution framework; service state capture and rollback; file/registry state capture and rollback; destructive cleanup policy | No scope exception | `Control Panel Settings Decomposition` | Yes |
| `write-cache-buffer-flushing` | Write Cache Buffer Flushing | Windows | `source-ultimate/6 Windows/20 Write Cache Buffer Flushing.ps1` | `28891440103D710F66F73620D944A9F29174B0DCCC211DCDE1008D694BBC90E2` | Implemented in Phase 47 | File/registry state capture and rollback | Phase 47 preserves Apply with exact value capture and refuses source Default because it deletes complete device `Disk` subkeys. | Future Restore would require a reviewed captured-state selection flow; Default remains unavailable. | No scope exception; NVME Faster Driver must remain deleted | `Storage Write Cache Restore Review` | No |
| `cleanup` | Cleanup | Windows | `source-ultimate/6 Windows/22 Cleanup.ps1` | `13C3933AC95A9817E48C0FFA4971FB2CC2234F9783831C34675F9F529F2D507E` | Refused | Destructive cleanup policy | Recursively deletes temp data, `Windows.old`, `inetpub`, `PerfLogs`, and dumps with no approved rollback path. Phase 50 scope design: `docs/tool-designs/cleanup-scope-design.md`. | Destructive cleanup policy; file/registry state capture and rollback | No scope exception | `Cleanup Inventory and Confirmation` | Yes |
| `timer-resolution-assistant` | Timer Resolution Assistant | Advanced | `source-ultimate/8 Advanced/6 Timer Resolution Assistant.ps1` | `46098A6B38BA04DA4A5A962EDC9B7EEBF2742A158845FA82C183D865133D2E73` | Refused | Service state capture and rollback | Generates C# under `C:\Windows`, compiles a binary, creates/removes a service, changes protected timer registry state, and deletes scoped files without approved provenance or service rollback guarantees. Phase 55 scope design: `docs/tool-designs/timer-resolution-assistant-scope-design.md`. | Service state capture and rollback; file/registry state capture; compiler/generated artifact approval; destructive cleanup policy | No scope exception | `Timer Resolution Service Assistant` | Yes |
| `defender-optimize-assistant` | Defender Optimize Assistant | Advanced | `source-ultimate/8 Advanced/7 Defender Optimize Assistant.ps1` | `FA09439A4056CA16937B47AEA6D70092312513D92EC9DFA09CF62B1D625E0B92` | Refused | Safe Mode recovery/resume framework | Security-sensitive workflow using TrustedInstaller, Safe Mode, RunOnce, BCD edits, generated scripts, scheduled task changes, Defender/security registry mutation, and repeated reboots. Phase 54 scope design: `docs/tool-designs/defender-optimize-assistant-scope-design.md`. | Safe Mode recovery/resume framework; TrustedInstaller execution framework; service state capture and rollback; reboot/recovery workflow | No scope exception | `Defender Safe Mode Recovery Assistant` | Yes |
...
The Phase 45/129 DirectX artifact source review is documented in
`docs/directx-provenance-review.md`. Phase 129 implements source-equivalent
runtime while preserving the source URLs and recording both author-hosted
downloads as `NeedsBoostLabMirror`; no artifact provenance approval or mirror
substitution is added.

The Phase 46 Visual C++ provenance review is documented in
`docs/visual-cpp-provenance-review.md`. Phase 130 implements source-equivalent
controlled runtime. Visual C++ mirror/provenance work remains separate because all twelve source URLs
...
`updates-drivers-block` uses this foundation after Phase 112 for selected USB
`setupcomplete.cmd` file capture and selected captured-state Restore. It is no
longer a remaining placeholder. Broad Windows Updates, custom update-server,
live registry Unblock/Default, script execution, and host reboot branches remain
blocked unless Yazan changes the final scope decision.
...
Phase 112 preserves the source script's `shutdown /r /t 0` line inside the
generated USB `setupcomplete.cmd` for Windows Setup context only. No BoostLab
reboot workflow scope is approved or used.
