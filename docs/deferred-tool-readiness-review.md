...
| Driver Install Debloat & Settings | `driver-install-debloat-settings` | Graphics | Implemented in Phase 99 as controlled manual handoff; Phase 122 branch-scope decision recorded | Full Auto behavior downloads tools, installs NVIDIA/AMD/Intel drivers, imports profiles, removes components/services/tasks/processes/files, writes registry, and reboots | Download provenance, installer policy, driver rollback, reboot workflow, file/registry rollback, AppX policy, cleanup policy | Auto remains blocked: no exact three-branch source-equivalent implementation, no approved NVIDIA/AMD/Intel artifacts, no approved user-selected-driver validation, no approved driver/service/task/process scopes, no approved profile/AppX/package/cleanup/reboot scopes | Exact NVIDIA/AMD/Intel device/package scopes, exact artifact approvals, exact service/task/process scopes, exact profile/AppX/package/cleanup/registry scopes, exact reboot workflow | Pending exact parity implementation | Phase 99 manual handoff complete; Phase 122 requires all source branches before parity can be marked done | Manual handoff is implemented; future Auto must implement NVIDIA/AMD/INTEL source branches using `docs/tool-designs/driver-install-debloat-settings-scope-provenance-design.md` |
