...
and source-defined Apply/Default behavior only. Phase 96 promoted P0 State as
Path B step 4 with the same controlled NVIDIA-only registry/capture model.
Phase 97 promoted Msi Mode as Path B step 5 with controlled NVIDIA-only
display-device Enum registry targeting, pre-change registry state capture,
verification, and source-defined Apply/Default behavior only.
...
manual handoff only, then superseded in Phase 124 by source-equivalent
branch-selected runtime behavior for this tool only. Nvidia Settings was
promoted in Phase 94 as Path B step 2 controlled manual handoff only. HDCP was
promoted in Phase 95 as Path B step 3
...
behavior with NVIDIA-only targeting and capture before mutation. Msi Mode was
promoted in Phase 97 as Path B step 5 controlled display-device Enum registry
behavior with NVIDIA-only targeting and capture before mutation. The remaining
source-promoted script remains separate from official active/deferred BoostLab tools until a future catalog/placeholder phase
...
| `intake/missing-ultimate-scripts/Ultimate/5 Graphics/2 Driver Install Latest.ps1` | `41C9DEA9AA5D208C9ED1EB7F1512B24251FBF4DC01C6DE2858B5B1A26C631A2F` | Intake accepted for future source promotion | Phase 124 implements source-equivalent branch-selected runtime for NVIDIA, AMD, and INTEL in this tool only | `source-ultimate/_intake-promoted/Ultimate/5 Graphics/2 Driver Install Latest.ps1` | Yes | Yes, resolved by adding an explicit Graphics order 2 tool and moving Path A to order 3 | Active source-equivalent branch runtime; no reusable/global artifact approval, Default, Restore, or cross-tool AMD/Intel scope approved | Future reusable artifact provenance, production allowlists, driver rollback, Default, and Restore remain unapproved; Path B workflow design remains separate |
