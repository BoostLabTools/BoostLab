...
foreach ($tool in $tools) {
    ...
    $defaultActionIsBlockedUntilApprovedContract = (
        [string]$tool['Id'] -in @('bitlocker', 'installers', 'driver-install-debloat-settings', 'directx', 'visual-cpp', 'reinstall') -and
    ...
    $restoreActionIsBlockedUntilSelectedState = (
        [string]$tool['Id'] -in @('hdcp', 'p0-state', 'msi-mode', 'bitlocker', 'installers', 'driver-install-debloat-settings', 'directx', 'visual-cpp', 'reinstall') -and
