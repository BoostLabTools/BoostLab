...
Assert-BoostLabCondition ([int]$pathATool.Order -eq 3) 'Path A Driver Install Debloat & Settings must remain separate after Driver Install Latest.'
...
foreach ($remainingPathBTool in @('nvidia-settings', 'hdcp', 'p0-state', 'msi-mode')) {
...
$remainingSourcePromoted = @(
    $sourcePromotedFiles | Where-Object {
        $_.Name -notin @('1 Driver Clean.ps1', '2 Driver Install Latest.ps1')
    }
)
